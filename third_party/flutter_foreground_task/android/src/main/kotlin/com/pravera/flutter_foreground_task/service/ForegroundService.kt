...
class ForegroundService : Service(), MethodChannel.MethodCallHandler {
	...
	private fun registerBroadcastReceiver() {
		...
		// Android 14+（targetSdk 34+）：双参 registerReceiver 必须显式声明
		// RECEIVER_EXPORTED / RECEIVER_NOT_EXPORTED，否则前台服务启动即抛
		// SecurityException（"One of RECEIVER_EXPORTED or RECEIVER_NOT_EXPORTED
		// should be specified..."）导致整个进程崩溃。
		// 这两个 action 仅由本应用内部发送（通知按钮/通知点击），因此声明为
		// RECEIVER_NOT_EXPORTED；API 26-32 三参形式 flags 被忽略，行为等同旧式。
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
			registerReceiver(broadcastReceiver, intentFilter, Context.RECEIVER_NOT_EXPORTED)
		} else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
			registerReceiver(broadcastReceiver, intentFilter, 0)
		} else {
