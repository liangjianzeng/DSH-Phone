...
class SSHTransport {
  ...
  /// Whether zlib compression is active (enabled after authentication,
  /// matching OpenSSH's `Compression delayed`).
  bool _compressionEnabled = false;
  ByteConversionSink? _compressSink;
  ByteConversionSink? _decompressSink;
  final _compressOut = BytesBuilder();
  final _decompressOut = BytesBuilder();

  /// Negotiated compression algorithm ('zlib' or null for none).
  String? _compressAlgorithm;

  /// Whether zlib compression was negotiated in the key exchange.
  bool get compressionNegotiated =>
      _compressAlgorithm == 'zlib' || _compressAlgorithm == 'zlib@openssh.com';

  /// Whether per-channel compression (zlib@openssh.com) was negotiated.
  bool get perChannelCompression => _compressAlgorithm == 'zlib@openssh.com';

  /// Whether compression is currently active.
  bool get compressionEnabled => _compressionEnabled;

  /// Enable zlib compression (called by SSHClient after authentication).
  void enableCompression() {
    if (_compressionEnabled) return;
    _compressionEnabled = true;
    _compressOut.clear();
    _decompressOut.clear();
    _compressSink = ZLibEncoder().startChunkedConversion(_AccumulatorSink(_compressOut));
    _decompressSink = ZLibDecoder().startChunkedConversion(_AccumulatorSink(_decompressOut));
  }

  /// Compress a payload with the continuous zlib stream.
  Uint8List _compressData(Uint8List data) {
    _compressSink!.add(data);
    return _compressOut.takeBytes();
  }

  /// Decompress a payload with the continuous zlib stream.
  Uint8List _decompressData(Uint8List data) {
    _decompressSink!.add(data);
    return _decompressOut.takeBytes();
  }
  ...
  void sendPacket(Uint8List data) {
    ...
    // 认证后启用 zlib 压缩：压缩负载后再打包（与 OpenSSH 的
    // `Compression delayed` 行为对齐）。
  ...
  void _processPackets() {
    ...
    while (_buffer.isNotEmpty && !isClosed) {
      var payload = _consumePacket();
      if (payload == null) {
        break;
      }

      // 认证后收到的数据是 zlib 压缩的：解压后再分发给消息处理器。
      if (_compressionEnabled) {
        payload = _decompressData(payload);
      }
  ...
  Uint8List? _consumeEncryptedPacket() {
    ...
    final encryptedLen = 4 + packetLength;
    if (_buffer.length + _decryptBuffer.length < encryptedLen + macLength) {
      return null;
    }

    // 优化：一次性解密剩余整包（processAll 一次处理整段，减少逐块开销）。
    // 剩余加密字节数 = 整包加密长度 - 已解密字节（首块已在前面解密）。
    final remainingEnc = encryptedLen - _decryptBuffer.length;
    if (remainingEnc > 0) {
      final encrypted = _buffer.consume(remainingEnc);
      _decryptBuffer.add(_decryptCipher!.processAll(encrypted));
    }

    final packet = _decryptBuffer.consume(encryptedLen);
    final paddingLength = SSHPacket.readPaddingLength(packet);
    final payloadLength = packetLength - paddingLength - 1;
    _verifyPacketPadding(payloadLength, paddingLength);
  ...
  void _sendKexInit() {
    ...
    final message = SSH_Message_KexInit(
      ...
      compressionClientToServer: algorithms.compression,
      compressionServerToClient: algorithms.compression,
  ...
  void _handleMessageKexInit(Uint8List payload) {
    ...
    // 协商压缩：优先按通道 zlib@openssh.com，其次全局 zlib，否则 none。
    final remoteComp = isClient
        ? message.compressionServerToClient
        : message.compressionClientToServer;
    if (algorithms.compression.contains('zlib@openssh.com') &&
        remoteComp.contains('zlib@openssh.com')) {
      _compressAlgorithm = 'zlib@openssh.com';
    } else if (algorithms.compression.contains('zlib') &&
        remoteComp.contains('zlib')) {
      _compressAlgorithm = 'zlib';
    } else {
      _compressAlgorithm = null;
    }
