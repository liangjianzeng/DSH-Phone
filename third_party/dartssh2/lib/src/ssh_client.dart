...
class SSHClient {
  ...
  /// Whether zlib compression is currently active (after auth).
  bool get isCompressionActive => _transport.compressionEnabled;
  ...
  void _handleUserauthSuccess() {
    ...
    // 认证完成后启用压缩（与 OpenSSH 的 Compression delayed 对齐）。
    // 仅对全局 zlib 启用传输级压缩；zlib@openssh.com 按通道压缩，
    // 由各通道控制器自行处理。
    if (_transport.compressionNegotiated && !_transport.perChannelCompression) {
      _transport.enableCompression();
    }
    _keepAlive?.start();
  }
  ...
  void _handleGlobalRequest(Uint8List payload) {
    printTrace?.call('<- $socket: SSH_Message_Global_Request');
    try {
      ...
      // OpenSSH 9.6 会发送新格式全局请求（如 ping@openssh.com /
      // kex-strict），dartssh2 解析可能失败；此时仅回 Request_Failure 保活，
      // 避免整个连接崩溃导致隧道掉线。
      _sendMessage(SSH_Message_Request_Failure());
    }
  }
  ...
  SSHChannelController _acceptChannel({
    ...
    final channelController = SSHChannelController(
      ...
      printDebug: printDebug,
      // 按通道压缩：zlib@openssh.com 协商时启用（认证后打开的通道）。
      perChannelCompress: _transport.perChannelCompression,
    );
