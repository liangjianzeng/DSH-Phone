...
class SSHChannelController {
  ...
  /// 是否启用按通道 zlib 压缩（zlib@openssh.com）。
  final bool perChannelCompress;
  ...
  SSHChannelController({
    ...
    this.printDebug,
    this.perChannelCompress = false,
  }) {
    if (perChannelCompress) {
      _compressSink =
          ZLibEncoder().startChunkedConversion(_AccumSink(_compressOut));
      _decompressSink =
          ZLibDecoder().startChunkedConversion(_AccumSink(_decompressOut));
    }
  ...
  late final _uploadLoop = OnceSimultaneously(() async {
    while (true) {
      ...
      // 按通道压缩：出方向先压缩数据再打包发送。
      final outBytes = perChannelCompress ? _compressData(data.bytes) : data.bytes;
...
/// 把 zlib 输出累积进 [BytesBuilder] 的转换 sink。
class _AccumSink extends ByteConversionSink {
  _AccumSink(this.builder);
